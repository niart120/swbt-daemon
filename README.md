...
リリース版は [GitHub Releases](https://github.com/niart120/swbt-daemon/releases) から入手します。各 release の Assets から Windows x86_64 用の zip と checksum を取得します。

Windows artifact の名前:

```text
swbt-daemon-v<version>-windows-x86_64.zip
swbt-daemon-v<version>-windows-x86_64.zip.sha256
```

zip には `swbt-daemon.exe`、診断用の `swbt-debug-client.exe`、README、license、third-party notice、manifest が入っています。BTstack を link する Windows 配布物であるため、MIT-only artifact とは扱いません。

source から build する場合は [docs/development.md](docs/development.md) を参照してください。
...
GitHub Release から zip と `.sha256` を取得し、checksum を確認してから展開します。

Windows PowerShell:

```console
Get-FileHash .\swbt-daemon-v<version>-windows-x86_64.zip -Algorithm SHA256
Get-Content .\swbt-daemon-v<version>-windows-x86_64.zip.sha256
```

展開後は `bin\swbt-daemon.exe` を実行します。

source checkout から build する場合:
