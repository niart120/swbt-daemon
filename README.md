...
| 項目 | 現在の状態 | 根拠 |
|---|---|---|
| 既定起動 | `swbt-daemon` は既定で production backend を選ぶ。`--adapter-location` 未指定では Bluetooth アダプターを開く前に失敗する。 | `apps/swbt-daemon/main.c`, `work-units/complete/local_074/DAEMON_LAUNCH_MODE_FLAGS.md`, `work-units/complete/local_077/ADAPTER_SELECTOR_GUARD.md` |
| noop 起動 | Bluetooth アダプターを開かない test / smoke では `--backend noop` または `--backend=noop` を明示する。 | `swbt/daemon/launch_options.c`, `tests/daemon_launch_options_test.c` |
...
| architecture cutover | daemon logical state は `swbt_domain_t`、lifecycle / cleanup は `swbt_daemon_process_t`、production BTstack 操作は `swbt_btstack_production_ports_t` を経由する。旧 session、mailbox、runtime predecessor、production backend ops、`swbt_core` は削除済み。 | `spec/architecture/daemon-architecture-cutover.md`, `work-units/complete/local_056/ARCHITECTURE_CUTOVER.md`, `work-units/complete/local_083/MODULE_RENAME_AND_PLACEMENT_CLEANUP.md` |
...
`production` は現行 CLI の backend 名です。release 品質や実機承認済みを意味しません。

| 区分 | 指定 | 役割 |
|---|---|---|
| 既定 | 指定なし | production backend を選ぶ。`--adapter-location` がない場合は adapter open 前に失敗する。 |
| test / smoke | `--backend noop` | Bluetooth アダプターを開かない。 |
| Windows adapter selector | `--adapter-location winusb:<location-path>` | 指定した USB location path に一致する WinUSB device interface だけを open 対象にする。 |
| Linux adapter selector | `--adapter-location libusb:<bus>:<port-path>` | BTstack の libusb bus + port path selector に渡す。Linux 実機経路は未確認。 |
| 診断 | `--trace-path <path>`、`--hci-dump-path <path>`、`--crash-dump-path <path>` | trace、HCI dump、crash dump の出力先を指定する。 |
| runtime config | `SWBT_IPC_HOST`、`SWBT_IPC_PORT`、`SWBT_REPORT_PERIOD_US`、`SWBT_DEVICE_INFO_PROFILE` | IPC endpoint、report period、device info profile を上書きする。 |

実機へ触れる production 起動では、ユーザが Bluetooth adapter open、Switch pairing、HID advertising、report loop、IPC input、cleanup confirmation の承認範囲を明示してから実行します。専用 USB Bluetooth ドングルを使い、内蔵 Bluetooth や普段使いのドングルを対象にしません。Windows native では、対象ドングルの driver が WinUSB に割り当たっていることと `winusb:<location-path>` selector を記録します。

`SWBT_DAEMON_BACKEND`、`SWBT_RUN_HARDWARE`、`SWBT_HARDWARE_APPROVED` は current implementation の起動 mode selector ではありません。実機承認は実装上の環境変数 gate ではなく、`hardware-harness` と `spec/operations/windows-native-preflight.md` の運用 gate として扱います。
...
このモデルでは、責務を次のように分けます。production backend は実装済みですが、実機へ触れる起動は `--adapter-location` による adapter selector と、実行前の運用承認を必要条件にします。

- `swbt_domain_t` は single active owner、latest controller state、sequence、rumble、status counters を所有します。
- `swbt/control` は public C ABI と daemon IPC から来る操作を domain / runtime へ渡します。
- `swbt/ipc` は transport、framing、JSON codec、client identity、command mapping を扱います。
- `swbt_runtime_host_t` は HID registration、output handler、report timer、neutral shutdown、runtime resource status を扱います。
- `swbt_daemon_process_t` は IPC runner と runtime host の composition root であり、startup / shutdown / cleanup ordering を構成します。
- `swbt_daemon_production_runner_t` は production lifecycle をまとめ、BTstack-facing 操作は `swbt_btstack_production_ports_t` 経由で呼びます。
- BTstack production implementation は pinned BTstack の callback、HID send、run loop、HCI transport を扱い、domain の内部状態を直接持ちません。
...
`just debug` は linux-debug の unit test executable target を build して CTest を実行します。`format-check` と `clang-tidy` は実行しません。formatter / linter を含める場合は `just verify` を使います。

test executable だけを build する場合:
...
`just verify` は `format-check`、`clang-tidy`、fresh configure からの debug test target build/test、ASan、Windows cross build を実行します。
...
`just clean` は Git の除外対象である `build/` と `cmake-build-*` だけを削除します。`tmp/`、Dev Container、submodule には触れません。
...
- `pre-push` は `just verify` を実行します。`format-check`、`clang-tidy`、fresh configure からの debug test target build/test、ASan、Windows cross build を含みます。
- `SWBT_FAST_PRE_PUSH=1` を指定すると `just debug` だけを実行します。これは test target の build/test だけを行い、`format-check` と `clang-tidy` は含みません。
...
swbt/support/           diagnostics、platform support
