...
`対象外` は、開始時点でこの work unit に含めない境界を書く。対象外は失敗や未完了ではない。後続作業として扱う具体的な観測や判断材料がないもの、または project scope として扱わないものはここに置く。
...
`先送り事項` は、作業中に見つかったがこの work unit では完了しない follow-up を置く。後続 work unit の source として使う意思があるもの、TDD Test List で `deferred` にしたもの、spec の未解決事項へ上げるほど安定していないが忘れると困るものはここに置く。各 item には、観測、先送り理由、後続 source として使うための次の置き場を書く。何もなければ `none` と書く。

`対象外` と `先送り事項` の違いは、後で作業を起こせる source になっているかで判断する。たとえば「複数 controller 同時接続は v0 scope では扱わない」は対象外でよい。一方、「TDD skill を分割する候補があり、候補名と判断軸も分かっているが今回実装しない」は先送り事項に置く。
