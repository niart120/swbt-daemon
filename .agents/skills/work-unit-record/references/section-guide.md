...
`対象外` は、開始時点でこの work unit に含めない境界を書く。対象外は失敗や未完了ではない。対象外にした項目も、後続 work unit の source になり得る。
...
`先送り事項` は、作業中に見つかったがこの work unit では完了しない follow-up を置く。TDD Test List で `deferred` にしたもの、spec の未解決事項へ上げるほど安定していないが忘れると困るもの、対象外にした領域から後続で追跡する具体的な work candidate を残す場合はここに置く。各 item には、観測、先送り理由、後続 source として使うための次の置き場を書く。何もなければ `none` と書く。

`対象外` と `先送り事項` の違いは、後続 source になれるかではなく、record 内の役割で判断する。`対象外` は今回の完了条件を閉じる境界である。`先送り事項` は後で追跡する follow-up record である。同じ文面を両方に置かず、対象外にした領域から後続候補を残す場合は、先送り事項側に観測、判断理由、次の置き場を持つ別 item として書く。
