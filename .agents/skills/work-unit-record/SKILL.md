...
`対象外` には、今回の完了条件に含めないだけで、後続 work unit の source として起こす具体性がないものを置く。後続で扱う意思、観測、判断材料、次の置き場があるものは、`対象外` ではなく `先送り事項` に置く。

先送り事項には、なぜ今扱わないか、後続の source として使える観測、次の置き場を残す。候補は `work-units/wip/local_{nnn}/...`、`spec/dev-journal.md`、`spec/operations/`、`spec/architecture/`、`spec/protocols/`、`docs/` のいずれかにする。TDD Test List の `deferred` item は、対応する先送り事項か、先送り不要と判断した理由へ辿れるようにする。
