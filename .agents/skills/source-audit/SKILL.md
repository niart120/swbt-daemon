...
Switch HID、BTstack、Bluetooth アダプターの挙動を `swbt-daemon` の事実として扱う前に、この skill を使う。
...
- packet layout や daemon の挙動を characterise するローカル swbt test。
...
大きな判断は `spec/wip/local_{nnn}/FEATURE_NAME.md` の spec に記録する。
小さな観測や先送り事項は `spec/dev-journal.md` に記録する。
実機測定値は `docs/hardware-test-log.md` に記録する。

## 安全ルール

- work unitが submodule fork または upstream patch の必要性を明示的に判断していない限り、`vendor/btstack` を直接変更しない。
