---
name: tdd-workflow
description: "CMake、Ninja、CTest、sanitizer preset、work unit record の source / use case / TDD Test List を使う swbt-daemon の TDD orchestration skill。Codex が TDD、ユースケースからの TDD test list 作成、red/green/refactor、green 後の refactor-done / refactor-skipped 判断、tidy-first 判断、Test Desiderata review、C unit test 追加、Switch packet behavior の characterization、protocol/IPC behavior の実装を求められたときに使う。必要に応じて tdd-test-list、tdd-one-cycle、refactor-after-green、tidy-first、test-desiderata-review に分岐する。"
---
...
| green 後に小さな構造変更を行う、または行わない判断を記録する | `../refactor-after-green/SKILL.md` |
...
5. green 後に構造変更を行う、または行わない判断を記録する場合は `../refactor-after-green/SKILL.md` を読む。
6. 構造変更の分類や時期に迷う場合は `../tidy-first/SKILL.md` を読む。
7. test が複数の期待、characterization、hardware-gated、壊れやすい assertion を含む場合は `../test-desiderata-review/SKILL.md` を読む。
8. work unit record の TDD Test List、TDD status、検証、先送り事項を更新する。
...
- green 後の小さな構造変更が中心の場合は、`refactor-after-green` を読んで `refactor-done`、`refactor-skipped`、`deferred` の判断を残す。
- refactor、cleanup、下準備の分類や時期が中心の場合は、`tidy-first` を読んで behavior change と structure change を分類してから差分を作る。
...
`notes` には、読んだ分割 skill、追加した Test List item、refactor 判断、tidy 判断、Test Desiderata review の trade-off、実機未実行理由を必要に応じて残す。
