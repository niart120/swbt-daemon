...
static int domain_acquires_and_rejects_owners(void) {
    ...
    failed += expect_eq_int(swbt_domain_acquire(app, 1001u), SWBT_DOMAIN_OK);
    failed += expect_eq_int(swbt_domain_acquire(app, 2002u), SWBT_DOMAIN_ERROR_OWNER_BUSY);
...
static int stale_sequence_does_not_update_state(void) {
    ...
    failed += expect_eq_int(swbt_domain_set_state(app,
                            ...
                            SWBT_DOMAIN_ERROR_STALE_SEQUENCE);
...
static int expect_active_state(const swbt_domain_t *app, expected_active_state_t expected) {
    swbt_domain_status_snapshot_t status;
    int failed = 0;
...
static int expect_neutral_state(const swbt_domain_t *app) {
    swbt_domain_status_snapshot_t status;
    int failed = 0;
...
static int set_active_state(swbt_domain_t *app) {
    swbt_state_t state = swbt_state_neutral();
    int failed = 0;
...
static int revoke_reasons_share_neutral_policy(void) {
    static const swbt_domain_revoke_reason_t reasons[] = {
        SWBT_DOMAIN_REVOKE_RELEASE,
    ...
    failed += expect_eq_int(swbt_domain_revoke(app,
                                               (swbt_domain_revoke_options_t){
                                                   .reason = SWBT_DOMAIN_REVOKE_RELEASE,
                                                   .client_id = 2002u,
                                               }),
                            SWBT_DOMAIN_ERROR_NOT_OWNER);
...
int main(void) {
    int failed = 0;
    failed += domain_acquires_and_rejects_owners();
