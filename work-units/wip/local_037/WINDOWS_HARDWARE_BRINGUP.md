...
| shutdown neutral send software gate | shutdown request clears daemon owner state and calls `hid_device_send_interrupt_message` through the report timer adapter before HCI power-off | implementation fact / unverified hardware hypothesis | `swbt/daemon/runtime.c`, `swbt/daemon/production_backend.c`, `swbt/btstack_bridge/input_report_timer_adapter.c`, `tests/daemon_production_backend_test.c` | unit / cross build pass; Switch-facing packet still requires post-fix hardware rerun |
...
| todo | shutdown while Button A owner is held emits a trailing neutral input report before HCI power-off | edge | hardware | yes |
...
- shutdown neutral software gate 後の実機 rerun。期待する HCI dump は `Button A 080000`、trailing neutral `000000`、`hci_power_control: 0` の順である。
...
- 観測: shutdown neutral software gate は unit / cross build で pass したが、shutdown 直前の同期 HID interrupt send が Switch-facing trailing neutral report として出るかは未観測である。
  先送り理由: 実機 HCI dump、専用 dongle、driver state、Switch firmware、daemon log が必要であり、software gate だけでは Switch-facing behavior を証明できない。
  次の置き場: post-fix 実機 rerun 後に `docs/hardware-test-log.md` とこの work unit record へ記録する。
...
- [ ] shutdown neutral software gate 後の実機 rerun で trailing neutral report を確認した。
