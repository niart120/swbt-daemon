...
2026-06-20 の準備では、入力経路として NyXpy handoff を使う方針を選んだ。ただし現行の `apps/swbt-daemon/main.c` は `swbt_daemon_runtime_noop_backend()` を使うため、現在の `swbt-daemon.exe` は Bluetooth adapter、IPC listener、HID advertising、periodic report loop を起動しない。この状態では NyXpy から実機入力反映を検証できないため、実機 daemon run の前に production backend entrypoint の準備状態を別途満たす。
...
- 実機 bring-up の開始順は `spec/operations/windows-hardware-bringup-sequence.md` に従う。`local_038` と `local_042` は完了済み software gate であり、残る `local_043` の後に、この record は実機観測と hardware log を扱う。
...
- `just windows-cross`: pass。`windows-mingw-debug` preset、`SWBT_BACKEND=windows-winusb`、`swbt-daemon.exe` と `swbt-debug-client.exe` の link 成功。
- `git ls-tree HEAD vendor/btstack`: `075a0780f0fad7ff67d58ac19f46e8953656a752`。
- `apps/swbt-daemon/main.c` と `spec/architecture/daemon-runtime-boundaries.md` の確認: 現行 entrypoint は no-op backend であり、実 Bluetooth adapter、実 IPC listener、BTstack run loop、HID advertising、periodic report loop は起動しない。

未実行:

- daemon run、Switch pairing、HID advertising、report loop。理由は、実機承認前であり、かつ現行 `swbt-daemon.exe` が hardware bring-up 用の production backend entrypoint ではないためである。
- NyXpy macro 実行。理由は、Project NyX 側の作業であり、かつ接続先となる実 IPC listener が swbt-daemon 側でまだ起動しないためである。
...
- `swbt-daemon.exe` が no-op backend のままである場合は実機 daemon run に進まない。
- IPC listener の endpoint を daemon log から確認できない場合は NyXpy macro を実行しない。
...
- 観測: 現行 `swbt-daemon.exe` は no-op backend を使い、Bluetooth adapter、実 IPC listener、BTstack run loop、HID advertising、periodic report loop を起動しない。
  ...
  次の置き場: `work-units/complete/local_038/BTSTACK_SEND_READY_INTEGRATION.md`、`work-units/complete/local_042/PRODUCTION_IPC_RUNNER_AND_STATE_SYNC.md`、`work-units/wip/local_043/PRODUCTION_DAEMON_BTSTACK_ENTRYPOINT.md`。
