...
- shutdown 時に trailing neutral report を Switch-facing に送る改善。現状の実機観測では Button A held owner のまま HCI power-off へ進む。
