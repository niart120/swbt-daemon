...
| todo | `0x21` NFC/IR MCU config subcommand handling is source-audited and either ACKed for pairing progress or explicitly deferred with observed impact | characterization | hardware / unit | yes |
...
- held-state IPC input の Switch UI 画面遷移。理由は、trigger elapsed rerun で controller 1 枠と `SET_PLAYER_LIGHTS` は確認できたが、NyX capture は L+R prompt のままで、A など明確な画面遷移を起こす入力確認はまだ実行していないためである。
