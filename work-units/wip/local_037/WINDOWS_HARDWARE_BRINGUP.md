...
- Switch UI 上の controller 採用と held-state IPC input の Switch UI 反映。理由は、reply periodic holdoff fix 後も Switch2 22.1.0 の画面は `L + R` prompt のままで、`0x10` SPI read や `0x03` report mode へ進んでいないためである。
