...
この work unit は Switch request device info reply bytes と identity policy を扱うため、`source-audit` が必須である。現時点では監査未完了であり、`mizuyoukanao-pro` の値をそのまま規定 profile として採用しない。

### 根拠監査

| 項目 | 値 | 根拠 | source | status |
|---|---:|---|---|---|
| existing default profile reply | firmware `04 00`, controller type `03`, marker `02`, local BD_ADDR, tail `01 01` | implementation fact | `swbt/switch/switch_device_info.c`, `spec/protocols/switch-hid-core.md` | not source-audited as project default |
...
| swbt-owned profile name and bytes | 未定 | unverified hypothesis | none | blocked on source audit |

### 未解決事項

- 規定 profile の bytes は、upstream / reverse engineering notes / swbt 実機ログ / characterization test のどれを根拠に stable とするかを決める。
...
|---|---|---|---|---|
| todo | `SWBT_DEVICE_INFO_PROFILE` 未指定時に swbt-owned profile が daemon の既定 device info として適用される | new | unit | no |
| todo | `SWBT_DEVICE_INFO_PROFILE` が swbt-owned profile 名を明示的に受理する | new | unit | no |
| todo | unknown profile は reject され、既存 config を破壊しない | regression | unit | no |
| todo | request device info reply が swbt-owned profile の firmware、controller type、marker、local BD_ADDR、tail を返す | new | unit | no |
| todo | `SWBT_DEVICE_INFO_PROFILE=mizuyoukanao-pro` は reject され、互換 selector としても残らない | regression | unit | no |
...
- [ ] source audit で規定 profile の値と根拠を分類した。
- [ ] swbt-owned profile 名を決めた。
- [ ] 規定 profile を daemon の既定値にした。
- [ ] `mizuyoukanao-pro` を互換期間なしで削除した。
