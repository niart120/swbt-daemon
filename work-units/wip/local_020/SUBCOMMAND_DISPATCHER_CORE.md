...
- state、manual pairing、request device info、set report mode、SPI flash read、enable IMU、enable vibration、get regulated voltage の初期 dispatcher 方針を整理する。
...
- report mode、IMU、vibration の状態は session state に分け、IPC の controller state へ混ぜない。
...
|---|---|---|---|---|
| todo | audited simple subcommand is dispatched to a reply action without BTstack send | new | unit | no |
