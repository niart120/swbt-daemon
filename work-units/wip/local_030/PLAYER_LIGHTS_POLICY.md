...
- get player lights request に対する reply payload を監査済み policy に従って生成する。
...
- get request は現在の policy state から reply payload を組み立てる。
...
| todo | audited set player lights payload updates policy state | new | unit | no |
| todo | get player lights returns reply payload from current policy state | new | unit | no |
