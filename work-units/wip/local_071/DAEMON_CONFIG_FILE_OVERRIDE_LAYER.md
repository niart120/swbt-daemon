...
- `work-units/wip/local_065/BONDED_RECONNECT_PERSISTENCE.md`: bond cache 保存先は daemon config abstraction に載せ、新しい起動時環境変数を増やす場合は一時的入力として扱う方針を記録した。
...
- 期待する観測結果:
  ...
  - 設定ファイルに書いた runtime / diagnostic / bond cache 設定が daemon config に反映される。
...
- runtime override、diagnostic sink、bond cache path の設定ファイル対応を検討する。
...
- bond cache persistence の実装そのもの。必要な config key は `local_065` と整合させる。
...
ただし bond cache path を追加する場合、BTstack link key DB の接続や実機 reconnect は `local_065` の根拠監査と実機実行条件に従う。
...
- bond cache path は設定ファイル向きである。環境変数を増やす場合でも、一時 override として削除条件を同時に記録する。
...
| todo | bond cache path can be supplied through daemon config abstraction without adding a permanent startup-only environment dependency | new | unit | no |
...
- `git status --short`: `work-units/wip/local_065/BONDED_RECONNECT_PERSISTENCE.md` に既存の未コミット差分あり。
...
実機が必要になるのは、設定ファイル移行と同時に production backend の既定化、report period 既定値変更、device info payload 変更、bonded reconnect 実装を行う場合である。その場合は別 work unit または `local_065` の実機実行条件に従う。
...
- 観測: CLI subcommand による `show-config` / `validate-config` / `reset-bond-cache` は有用である。
  先送り理由: この work unit は daemon 起動設定の合成 layer を先に固定する。CLI 管理 surface は scope が広い。
  次の置き場: 後続 work unit。

## 13. チェックリスト

- [x] source を user request、`local_045`、`local_065`、`docs/status.md` から特定した。
