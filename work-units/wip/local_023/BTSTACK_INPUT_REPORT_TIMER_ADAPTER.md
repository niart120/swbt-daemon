...
|---|---:|---|---|---|
| BTstack run loop timer API | 未確定 | existing source fact must be rechecked | `spec/references/btstack-periodic-input-report-core.md`, `vendor/btstack/src/btstack_run_loop.c` | pending |
| BTstack HID can-send / interrupt send API | 未確定 | existing source fact must be rechecked | `spec/references/btstack-periodic-input-report-core.md`, `vendor/btstack/src/classic/hid_device.h` | pending |
| report period | configurable default | design policy with existing audit | `spec/references/btstack-periodic-input-report-core.md` | not hardware-proven |
| Switch report loop acceptability | 未記録 | hardware observation required | `docs/hardware-test-log.md` | not run |
...
未監査の BTstack timer 値や backend behavior を実装の安定事実として扱わない。
