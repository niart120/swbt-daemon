...
- source: `work-units/wip/local_084/PRODUCTION_RUNNER_DECOMPOSITION_PLAN.md` and this
  work unit.
- use case: BTstack IPC pump port の start が失敗した場合、先に start 済みの daemon
  IPC runner を停止し、listen socket と control binding を残さない。
- item: BTstack IPC pump start failure still stops daemon IPC runner.
- state: green.
- command: `just format`
- result: pass.
