...
- `swbt_switch_spi_t` に audited seed profile を書き込む helper を追加する。
...
- unseeded range は existing virtual SPI の erased byte policy を維持する。
...
- seed data は固定配列ではなく、監査済み profile 構造から書き込む。
...
|---|---|---|---|---|
| todo | audited seed profile writes expected ranges through `swbt_switch_spi_write` | new | unit | no |
...
Switch が seeded virtual SPI を受け入れるかを確認する段階では実機検証が必要である。
