...
|---|---|---|---|---|
| todo | docs state that binary release is out of current scope or list exact release gate prerequisites | characterization | docs | no |
| todo | BTstack-containing artifacts are not described as MIT-only | regression | docs | no |
...
未実行。起票のみで、docs / notice はまだ変更していない。
