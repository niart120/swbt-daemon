...
- `tdd-workflow`、`work-unit-record`、`AGENTS.md` の skill / status 記述を更新する。
...
- notes: read `skill-creator`, `work-unit-record`。`tdd-one-cycle` から `refactor-after-green` へ導線を張り、`refactor-skipped` を状態値として追加した。
...
- `rg -n "state: red \| green \| refactor-done \| deferred|todo`、`red`、`green`、`refactor-done`、`deferred" .agents AGENTS.md`: pass, no old status-only list remained in active skill docs。
