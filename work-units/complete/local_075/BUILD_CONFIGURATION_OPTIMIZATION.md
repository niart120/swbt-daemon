...
- PR CI run 162: fail。`CCACHE_DIR` に host 側 `${{ github.workspace }}/.ccache` を渡していたため、container 内の `ccache` が書き込めず `ccache: error: Permission denied` になった。
- PR CI fix: `CCACHE_DIR` を container 内 workspace path の `/workspaces/swbt-daemon/.ccache` に変更した。
- main CI run 164: fail。`Verify in Dev Container` は success したが、`devcontainers/ci` の post step で GHCR へ image push する際に Docker login しておらず `unauthorized` になった。
- main CI fix: `main` push のときだけ `GITHUB_TOKEN` で GHCR に Docker login してから `devcontainers/ci` を実行する。
