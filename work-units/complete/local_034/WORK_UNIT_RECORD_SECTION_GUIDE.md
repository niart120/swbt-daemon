...
- agent が `対象外` と `先送り事項` で迷ったとき、後続 work unit の source として扱える具体性があるかで判断できる。
...
| refactor-done | target out-of-scope and deferred items are distinguished by later-source readiness | regression | docs | no |
...
- item: target out-of-scope and deferred items are distinguished by later-source readiness。
- state: refactor-done。
- commands:
