...
- ユーザ指摘。`tdd-workflow` skill 分割の話は対象外ではなく先送り事項に移すべきという判断。
- ユーザ指摘。対象外自体も後続 work unit の source になり得るため、対象外と先送り事項を source 化できるかどうかで区別するのは不適切という判断。
...
- agent が `対象外` と `先送り事項` で迷ったとき、後続 source になれるかではなく、現在の完了条件を閉じる境界か、後で追跡する follow-up record かで判断できる。
...
- `対象外` と `先送り事項` の違いを skill、reference、operations spec に明記する。
- `tdd-workflow` skill 分割の検討を `対象外` から `先送り事項` へ移す。
- 対象外も後続 work unit の source になり得ることを明記する。
...
| refactor-done | target out-of-scope items can become later sources while deferred items remain follow-up records | regression | docs | no |
| refactor-done | tdd-workflow skill split discussion is recorded as a deferred item | regression | docs | no |
...
- `tdd-workflow` skill の分割検討。
  - 観測: `tdd-test-list`、`tdd-one-cycle`、`tidy-first`、`test-desiderata-review` への分割候補がある。
  - 先送り理由: 今回は work unit record の構造と記載粒度を直す work unit であり、TDD skill の設計変更までは含めない。
  - 次の置き場: `spec/operations/work-unit-spec-tdd-flow.md` の未解決事項を source として、必要になった時点で別 work unit を立てる。
...
- [x] `対象外` と `先送り事項` の違いを明文化した。
- [x] `tdd-workflow` skill 分割の検討を先送り事項に移した。
- [x] 対象外も後続 work unit の source になり得ることを明文化した。
- [x] 根拠監査の状態を記録した。
- [x] 実機状態を記録した。
