...
規定 profile 名は `swbt-pro` とする。`swbt-pro` は現行の既定 reply bytes を project-owned profile として名前付けし、daemon の既定値にする。Bluetooth address は固定値ではなく、既存実装どおり runtime / production backend が得た local BD_ADDR を使う。
...
この record では、`規定 profile` は `swbt-daemon` が定める project-owned profile を指し、`既定値` は daemon が設定未指定時に使う選択を指す。規定 profile は daemon の既定値にする。
...
| refactor-skipped | `spec/protocols/switch-hid-core.md` が規定 profile と削除済み実験 profile を分けて説明する | regression | docs | no |
| refactor-skipped | `README.md` と `docs/status.md` が production 実行例で Mizuyoukanao 由来の profile を正規値として提示しない | regression | docs | no |
| deferred | swbt-owned profile で Switch 2 pairing 画面の Button A 入力反映を確認する | characterization | hardware | yes |
...
- 未決定のまま残す表現、互換を維持する表現、環境変数選択に留める表現の検索: no matches。
- `git diff --check -- work-units/complete/local_048/SWBT_DEVICE_INFO_PROFILE_DEFINITION.md`: pass。
...
- docs green: `README.md`、`docs/status.md`、`spec/protocols/switch-hid-core.md`、`spec/references/switch-subcommand-dispatcher-core.md` を `swbt-pro` 規定 profile と `mizuyoukanao-pro` 削除済み方針に更新した。
- docs check: 正規値として `mizuyoukanao-pro` を提示する旧表現の検索は no matches。`rg -n "swbt-pro|SWBT_DEVICE_INFO_PROFILE" README.md docs/status.md spec/protocols/switch-hid-core.md spec/references/switch-subcommand-dispatcher-core.md` で current docs の `swbt-pro` 記述を確認した。`git diff --check -- README.md docs/status.md spec/protocols/switch-hid-core.md spec/references/switch-subcommand-dispatcher-core.md` pass。
- Refactor status: refactor-skipped。docs の current contract 更新で閉じており、文書構造の追加変更は不要と判断した。
- final format: `just format-check` は `tests/daemon_runtime_test.c` の clang-format 差分で fail した。`just format` を実行後、`just format-check` pass。
- final verification: `just test-debug` 32/32 pass。`just asan` 32/32 pass。`just windows-cross` pass。実機は今回の objective で対象外のため未実行。
...
- `swbt-pro` profile での Switch 2 pairing 画面 Button A 入力反映確認は、今回の objective が実機検証を除外しているため deferred とする。後続で実行する場合は、ユーザの明示承認、専用 USB Bluetooth ドングル、WinUSB 割り当て確認、`SWBT_DAEMON_BACKEND=production`、`SWBT_RUN_HARDWARE=1`、`SWBT_HARDWARE_APPROVED=1`、`docs/hardware-test-log.md` への記録を必要条件にする。

## 13. チェックリスト

- [x] source audit で規定 profile の値と根拠を分類した。
- [x] swbt-owned profile 名を決めた。
- [x] 規定 profile を daemon の既定値にした。
- [x] `mizuyoukanao-pro` を互換期間なしで削除した。
- [x] daemon config と device info reply のテストを更新した。
- [x] `spec/protocols/switch-hid-core.md`、`README.md`、`docs/status.md` を更新した。
- [x] 実機実行の要否と結果または未実行理由を記録した。
- [x] 検証コマンドと結果を記録した。
