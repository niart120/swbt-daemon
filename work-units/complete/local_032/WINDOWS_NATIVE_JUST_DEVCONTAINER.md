...
- 入力 / 状態: repository が Windows filesystem path `E:\documents\VSCodeWorkspace\swbt-daemon` にあり、Windows host に `just`、Dev Container CLI、Docker Desktop WSL2 backend がある。
...
Windows filesystem checkout では、`devcontainer up --workspace-folder E:\documents\VSCodeWorkspace\swbt-daemon` と `devcontainer exec --workspace-folder E:\documents\VSCodeWorkspace\swbt-daemon ...` が通る。
...
- state: green。
- commands: `just list-presets`, `just debug`, `$env:CTEST_ARGS='-N'; just test-debug`, `devcontainer exec --workspace-folder E:\documents\VSCodeWorkspace\swbt-daemon pwd`。
...
- PowerShell: 7.6.2。
- repository path: `E:\documents\VSCodeWorkspace\swbt-daemon`。
- `just`: `C:\Users\train\.cargo\bin\just.exe`, version `1.53.0`。
- Dev Container CLI: `C:\Users\train\AppData\Local\pnpm\bin\devcontainer.ps1`, version `0.87.0`。
...
- final `just list-presets`: pass, Dev Container CLI returned container `0d255ddc1b4f8193df4d1a915f21ebb8a04a936a56fde45b2e761ded55256178` and ran `cmake --list-presets` in `/workspaces/swbt-daemon`。
...
- `devcontainer exec --workspace-folder E:\documents\VSCodeWorkspace\swbt-daemon pwd`: pass, output `/workspaces/swbt-daemon`。
...
- second `just verify`: failed at `scripts/check-format.sh` with Git dubious ownership for `/workspaces/swbt-daemon`. Added container recipe preparation that registers the workspace as `safe.directory` when needed。
