...
| active reconnect | Switch address を既知状態として扱う reconnect request boundary は production adapter に追加済み。設定ファイル上の effective Switch address がある場合、daemon は power on 後に HID control PSM `0x11` / interrupt PSM `0x13` 付きの request を出し、BTstack 実装では `hid_device_connect()` を使う。pairing / connection からの address 自動取得と実機 reconnect は未実装 / 未検証。 | `swbt/btstack_bridge/production_adapter.h`, `swbt/btstack_bridge/production_btstack.c`, `swbt/daemon/production_backend.c`, `tests/daemon_production_backend_test.c`, `work-units/wip/local_072/ACTIVE_SWITCH_RECONNECT.md`。 |
