...
| architecture cutover | daemon logical state は `swbt_domain_t` に残り、`swbt_runtime_host_t` が domain lifetime、`swbt_control_t`、runtime resource lifecycle を構成する。`swbt_daemon_process_t` は IPC start / stop と runtime host start / stop の process lifecycle 境界である。production BTstack 操作は能力別 port group を持つ `swbt_btstack_production_ports_t` を経由する。旧 session、mailbox、runtime predecessor、production backend ops、`swbt_core`、process-owned runtime backend wrapper は source / tests / build graph から削除済み。H1 は dedicated adapter で pass。shutdown pending failure cleanup は `local_058` で固定済み。 | `spec/architecture/daemon-architecture-cutover.md`, `work-units/complete/local_056/ARCHITECTURE_CUTOVER.md`, `work-units/complete/local_057/ARCHITECTURE_CUTOVER_H1.md`, `work-units/complete/local_058/SHUTDOWN_NEUTRAL_RETRY_FAILURE.md`, `work-units/complete/local_061/PRODUCTION_ADAPTER_TABLE_DECOMPOSITION.md`, `work-units/complete/local_097/RUNTIME_HOST_COMPOSITION_OWNERSHIP.md`, `CMakeLists.txt` |
...
| `swbt/runtime/host.c` | domain / control / runtime resource composition、HID registration、output handler、report timer、neutral shutdown の実装根拠。 |
| `swbt/daemon/process.c` | IPC start / stop、runtime host start / stop、daemon lifecycle status、shutdown cleanup ordering の実装根拠。 |
