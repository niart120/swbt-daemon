...
| pairing-free active reconnect | `local_073` の link key DB rerun で、保存済み address と保存済み TLV-backed Classic link key DB から daemon initiated outgoing reconnect が `Connection_complete (status=0)`、PSM `0x11` / `0x13` の `L2CAP_EVENT_CHANNEL_OPENED status 0x0`、HID open、Button A smoke まで到達した。HCI dump は `responding to link key request` と `have link key db: 1` を記録し、`Connection_incoming`、`pairing started`、`pairing complete, status 00` は記録しなかった。 | `docs/hardware-test-log.md`, `work-units/wip/local_073/DAEMON_CLI_LAUNCH_MODE.md` |
