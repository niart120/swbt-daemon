function(swbt_configure_module_public_includes target)
    set(options)
    set(one_value_args)
    set(multi_value_args MODULES INCLUDE_REGEXES EXCLUDE_REGEXES)
    ...
    foreach(module IN LISTS SWBT_PUBLIC_INCLUDES_MODULES)
        ...
        foreach(header_path IN LISTS header_paths)
            ...
            if(header_name MATCHES "_internal\\.h$")
                continue()
            endif()
            file(RELATIVE_PATH relative_header "${CMAKE_CURRENT_SOURCE_DIR}/swbt" "${header_path}")
            set(include_header TRUE)
            if(SWBT_PUBLIC_INCLUDES_INCLUDE_REGEXES)
                set(include_header FALSE)
                foreach(pattern IN LISTS SWBT_PUBLIC_INCLUDES_INCLUDE_REGEXES)
                    if(relative_header MATCHES "${pattern}")
                        set(include_header TRUE)
                    endif()
                endforeach()
            endif()
            foreach(pattern IN LISTS SWBT_PUBLIC_INCLUDES_EXCLUDE_REGEXES)
                if(relative_header MATCHES "${pattern}")
                    set(include_header FALSE)
                endif()
            endforeach()
