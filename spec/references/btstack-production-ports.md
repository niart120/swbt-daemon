# BTstack Production Ports Source Audit
...
Classic HID Device / SDP API へ接続する production port implementation の根拠監査である。
...
| Port implementation include set | `classic/hid_device.h`, `classic/sdp_server.h`, `classic/sdp_util.h` | source fact | headers above | implementation contract |
| BTstack source selection | implementation consumes selected include dirs; backend link libs are not propagated by this work unit | implementation fact | `cmake/btstack_sources.cmake`; `spec/references/btstack-source-selection.md` | implementation compile boundary |

## 4. 実装判断

- production port implementation は `swbt/btstack_bridge/` に置き、public header へ BTstack header を含めない。
- implementation は `swbt_btstack_production_ports_t` の port table と `swbt_btstack_hid_registration_backend_t` 向けの registration config を返す。
- unit test は BTstack symbols を test stub で解決し、implementation が registration core から呼ばれることを確認する。
- `swbt_btstack_production_impl` target は selected BTstack include dirs と `swbt_btstack` を使う。production executable は aggregate target を介さず、この target と `swbt_daemon_process` を直接 link する。
- daemon main は production backend opt-in 時に `swbt_btstack_production_ports_btstack()` を渡す。実機 advertising、pairing、run loop の実行は hardware gate で扱う。
