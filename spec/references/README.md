...
- `btstack-hid-event-port-boundary.md`: BTstack HID event decoder、HID send port、timer port の根拠監査。
- `btstack-production-ports.md`: pinned BTstack へ接続する production port implementation の根拠監査。
...
- `spec/protocols/switch-hid-core.md` は Switch HID、subcommand、SPI、rumble、player lights、scheduler priority policy、production send path、実機観測境界の current spec として、このディレクトリの個別根拠監査を参照する。
- `spec/architecture/daemon-architecture-cutover.md` は domain、IPC、BTstack bridge、daemon process の current boundary として、BTstack 関連 reference と完了済み work unit record を参照する。
