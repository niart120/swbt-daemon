...
- adapter は `swbt_btstack_production_adapter_t` の function table と `swbt_btstack_hid_registration_backend_t` 向けの registration config を返す。
- unit test は BTstack symbols を test stub で解決し、adapter が registration core から呼ばれることを確認する。
- `swbt_btstack_production_adapter` target は selected BTstack include dirs と `swbt_btstack` を使う。production executable は aggregate target を介さず、この target と `swbt_daemon_host` を直接 link する。
- daemon main は production backend opt-in 時に `swbt_btstack_production_adapter()` を渡す。実機 advertising、pairing、run loop の実行は hardware gate で扱う。
