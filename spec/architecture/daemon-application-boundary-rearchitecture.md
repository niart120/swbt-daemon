...
| `ipc_session` は control lease、state、rumble、mailbox を持つ互換 wrapper である | implementation fact | `swbt/ipc/ipc_session.h`, `swbt/ipc/ipc_session.c` | current |
| release / disconnect / heartbeat timeout は `ipc_session` で neutral 化し、session-side neutral publish helper を共有する | implementation fact | `swbt/ipc/ipc_session.c` | current after `local_050` |
...
- `ipc_session` compatibility wrapper を `local_051` で application command API へ寄せても、既存 IPC JSON response と debug client の互換性を保つ。
