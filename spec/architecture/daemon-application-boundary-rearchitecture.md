...
`production_btstack` の IPC pump と `swbt_daemon_production_backend_ops_t` 返却は残る。これは実機済み production composition の互換 glue として扱い、`local_054` で host / build boundary の対象にし、`local_055` で削除または current spec 上の責務定義を要求する。
...
| BTstack production adapter は daemon IPC runner と production backend 型を参照する | implementation fact | `swbt/btstack_bridge/production_btstack.h`, `swbt/btstack_bridge/production_btstack.c` | remaining compatibility glue after `local_053`; tracked by `local_054` / `local_055` |
...
- `swbt_ipc_session_t` forwarding wrapper は `local_052` で JSON codec から分離済みである。lock、mailbox publish、rumble status の compatibility context を `local_055` で削除または責務定義する。
- `production_btstack` の IPC pump と production backend ops table 接続は `local_053` 後も残る。`local_054` で host / build boundary の検出対象にし、`local_055` で削除または current spec 上の責務定義を行う。
