...
- status protocol v1 の field 固定。これは `work-units/wip/local_039/DAEMON_STATUS_OBSERVABILITY_PROTOCOL.md` で扱う。
...
`client_seq` は Switch へ送る controller state ではなく、IPC command の順序と診断に使う配送メタデータである。

移行後の controller state は report 生成に必要な値だけを持つ。`seq` は set-state command または control lease 側の metadata として保持し、既存 IPC status の `last_seq` 相当は互換性を保つ。

最初の code slice では、report bytes を変えずに `client_seq` の参照範囲を狭める。`seq` を daemon protocol から削除する作業ではない。
...
最初の移行単位では、`control_lease` 相当の小さな型を IPC 非依存に抽出し、`ipc_session` は互換 wrapper として残す。release、owner disconnect、heartbeat timeout、shutdown は、最終的に同じ revoke 処理を通る。
...
現行の `production_btstack` は BTstack run loop から IPC runner を poll し、daemon production backend の ops table を返す。この構造は実機 bring-up を通すための実装として維持する。
...
- status / observability: `work-units/wip/local_039`。
...
| work unit | 目的 | 完了時の状態 |
|---|---|---|
| `local_050` | controller state と command metadata を分離し、IPC 非依存の control lease を作る | IPC session は互換 wrapper に薄くなる |
...
| `local_053` | BTstack port / typed event 境界を導入する | BTstack adapter は daemon IPC runner と production backend 内部型を直接所有しない |
| `local_054` | daemon host と CMake include boundary を分ける | composition root と target 境界で禁止依存を検出できる |
| `local_055` | cutover、acceptance、互換層削除を完了する | 旧 glue / wrapper / aggregate dependency が残っていない |
...
| 項目 | 根拠 | source | status |
|---|---|---|---|
| `client_seq` は `swbt_state_t` に含まれる | implementation fact | `swbt/switch/switch_controller_state.h` | current debt |
| `ipc_session` は owner、state、rumble、mailbox を持つ | implementation fact | `swbt/ipc/ipc_session.h`, `swbt/ipc/ipc_session.c` | current |
| release / disconnect / heartbeat timeout は `ipc_session` で neutral 化する | implementation fact | `swbt/ipc/ipc_session.c` | current |
| daemon runtime backend は IPC、HID、timer、reply queue、device info を横断する | implementation fact | `swbt/daemon/runtime.h` | current |
| production backend ops は IPC、platform、HID、timer、SSP、clock、power、run loop を横断する | implementation fact | `swbt/daemon/production_backend.h` | current |
| BTstack production adapter は daemon IPC runner と production backend 型を参照する | implementation fact | `swbt/btstack_bridge/production_btstack.h`, `swbt/btstack_bridge/production_btstack.c` | current |
| `swbt_core` は daemon、IPC、BTstack bridge、Switch protocol を単一 target に含める | implementation fact | `CMakeLists.txt` | current |
...
- `control_lease` 抽出後も、既存 IPC JSON response と debug client の互換性を保つ。
