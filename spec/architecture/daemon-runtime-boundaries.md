...
- production daemon が実 IPC listener と実 BTstack backend を runtime backend として接続する作業は未完成である。現行 entrypoint は noop backend で runtime lifecycle だけを通す。実 IPC listener と state synchronization は `work-units/wip/local_042/PRODUCTION_IPC_RUNNER_AND_STATE_SYNC.md`、production BTstack entrypoint は `work-units/wip/local_043/PRODUCTION_DAEMON_BTSTACK_ENTRYPOINT.md` で扱う。
