...
- production daemon が実 BTstack backend を runtime backend として接続する作業は未完成である。現行 entrypoint は noop backend で runtime lifecycle だけを通す。実 IPC runner と synchronized state boundary は `work-units/complete/local_042/PRODUCTION_IPC_RUNNER_AND_STATE_SYNC.md` で完了し、production BTstack entrypoint は `work-units/wip/local_043/PRODUCTION_DAEMON_BTSTACK_ENTRYPOINT.md` で扱う。
