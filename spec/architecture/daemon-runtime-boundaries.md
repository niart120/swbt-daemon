...
daemon は Bluetooth adapter、BTstack run loop、HID Device registration、output report handling、periodic input report scheduler、local IPC server を所有する。client は daemon IPC に controller state snapshot を送る。
...
| 項目 | 根拠 | source | status |
|---|---|---|---|
| daemon runtime entrypoint | implementation fact | `apps/swbt-daemon/main.c`, `swbt/daemon/runtime.*` | current |
