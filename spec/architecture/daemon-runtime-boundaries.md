...
| production daemon shutdown path | implementation fact / source audit | `swbt/daemon/production_backend.*`, `apps/swbt-daemon/main.c`, `work-units/complete/local_044/PRODUCTION_DAEMON_SHUTDOWN_PATH.md` | current software gate; hardware stop observation pending |
...
- production daemon が実 Bluetooth adapter を開く順序、実機停止時の cleanup、HID disconnect cleanup は fake backend だけでは証明できない。`work-units/wip/local_037/WINDOWS_HARDWARE_BRINGUP.md` で実機実行条件付きで扱う。
- production daemon が実 BTstack backend を runtime backend として接続する software gate は `work-units/complete/local_043/PRODUCTION_DAEMON_BTSTACK_ENTRYPOINT.md` で完了した。実 IPC runner と synchronized state boundary は `work-units/complete/local_042/PRODUCTION_IPC_RUNNER_AND_STATE_SYNC.md` で完了済みである。Switch pairing、HID advertising、report loop の実機 acceptability は `work-units/wip/local_037/WINDOWS_HARDWARE_BRINGUP.md` で扱う。
- BTstack can-send event と subcommand reply queue / periodic scheduler の software integration は `work-units/complete/local_038/BTSTACK_SEND_READY_INTEGRATION.md` で完了している。実機 HID session での acceptability は `work-units/wip/local_037/WINDOWS_HARDWARE_BRINGUP.md` で扱う。
