...
| production IPC runner is available but not wired into daemon entrypoint | implementation fact / unresolved item | `spec/architecture/daemon-runtime-boundaries.md` | runner complete in `local_042`; entrypoint wiring pending in `local_043` |
...
- `local_042` では C11 `atomic_bool` の短い spin lock で state handoff を固定した。BTstack run loop と同じ thread へ dispatch するか、別 loop で駆動するかは `local_043` の entrypoint composition で扱う。
