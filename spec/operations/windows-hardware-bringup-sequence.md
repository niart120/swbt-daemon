...
この spec は、既定では no-op backend の `swbt-daemon.exe` から、NyXpy が接続できる production daemon、BTstack-backed HID session、最後の実機記録までの work unit 順序を定める。初回 bring-up sequence は `local_037` で完了済みであり、実機の成功結果はこの spec ではなく `docs/hardware-test-log.md` に記録する。
