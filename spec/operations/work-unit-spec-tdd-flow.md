...
`tdd-workflow` skill は、TDD 全体の入口とし、実作業は `tdd-test-list`、`tdd-one-cycle`、`tidy-first`、`test-desiderata-review` に分ける。入口 skill は source -> use case -> TDD Test List -> one cycle -> tidy / desiderata review -> work unit record update の順序を案内する。分割 skill は、それぞれの入力、出力、検証、record 更新境界を明確にする。
