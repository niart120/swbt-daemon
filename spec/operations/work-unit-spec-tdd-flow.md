...
この spec は、`work unit`、`work unit record`、`spec`、TDD Test List の使い分けと作成順序を定める。
...
この spec は、新しい work unit record の作成、既存 wip work unit record の再開、operations / architecture / protocols の spec 作成判断、TDD Test List の作成、TDD cycle の進行、project-local skill の guidance に適用する。
...
**work unit record** は、work unit の source、use case、範囲、関連 spec、TDD Test List、検証、実機状態、根拠監査状態を束ねる記録である。
...
Refactor では、観測可能な振る舞いを変えずに構造を変更する。behavior change と structure change は、差分、説明、検証を分ける。
...
- `tdd-workflow` を複数 skill に分割するかどうかは未決定である。
- 分割する場合の候補は `tdd-test-list`、`tdd-one-cycle`、`tidy-first`、`test-desiderata-review` である。
