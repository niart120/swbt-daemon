...
`just debug` は linux-debug preset の configure、unit test executable build、CTest を実行する fast debug loop である。`just debug` は `format-check` と `clang-tidy` を実行しない。
`just debug` と `just build-debug` は通常の CMake configure を使い、ローカル反復時に build tree を毎回 fresh configure しない。

unit test executable だけを build する場合は `just build-tests-debug` を使う。これは CMake target `swbt_unit_tests` を build し、`swbt-daemon` executable、`swbt-debug-client` executable、共有 C ABI library のような test 実行に不要な top-level target は要求しない。
daemon 本体だけを確認する場合は `just build-daemon-debug`、debug client だけを確認する場合は `just build-debug-client` を使う。これらは linux-debug preset を configure した後、Ninja target を指定して build する。

CTest は既定で 8 並列で実行する。並列度を変える場合は `SWBT_CTEST_PARALLEL_LEVEL=<n>` を指定する。`CTEST_ARGS` は従来通り `-R <pattern>` などの絞り込みに使う。
Ninja の build 並列度は既定では Ninja に任せる。上限を指定したい場合は `SWBT_BUILD_PARALLEL_LEVEL=<n>` を使う。
...
`just verify` と `just verify-ci` の debug gate は fresh configure から始める。ローカル反復用の `just debug` を増分化しても、push 前と CI の gate は clean configure boundary を保つ。
...
`just list-presets` は `cmake --list-presets` の出力を表示する開発者向け recipe である。CMake presets を読めることだけを quiet に確認する hook 用 recipe は `just check-presets` とする。
...
format script を呼ぶ `just` recipe は、container 内で `bash scripts/format.sh` と `bash scripts/check-format.sh` を実行する。これにより、Windows filesystem checkout 上の executable bit や shebang 解決には依存しない。
...
`pre-commit` は staged diff の whitespace、`just check-presets`、staged C source がある場合の `just format-check` を確認する。
`pre-push` は通常 `just verify` を実行する。`just verify` は `format-check`、`tidy`、`debug`、`asan`、`windows-cross` を含む。
軽い push 前確認だけにしたい場合は `SWBT_FAST_PRE_PUSH=1` を使い、`just debug` だけを実行する。`SWBT_FAST_PRE_PUSH=1` は hook skip ではなく test target の build/test だけを行う明示 opt-in である。
hook 全体を明示的にスキップする場合は `SWBT_SKIP_HOOKS=1` を使う。

CI では Dev Container 内で `just verify-ci` を実行する。
CI は software gate であり、Bluetooth adapter open、Switch pairing、HID advertising、report loop は実行しない。`SWBT_RUN_HARDWARE` と `SWBT_HARDWARE_APPROVED` は current implementation の起動 mode selector ではない。
