...
`pre-push` は通常 `just debug` を実行し、`SWBT_FULL_PRE_PUSH=1` のとき `just verify` を実行する。通常の `pre-push` は build/test only であり、push 前に formatter / linter まで確認する場合は `SWBT_FULL_PRE_PUSH=1` を使う。
