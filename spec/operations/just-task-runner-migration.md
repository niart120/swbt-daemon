...
5. Windows native PowerShell からの `just <recipe>` は、`work-units/wip/local_032/WINDOWS_NATIVE_JUST_DEVCONTAINER.md` で実行可否を確認してから標準入口に含めるか判断する。
...
12. 置き換え済みの current spec が残る場合は、`状態` を `superseded` にして `spec/archive/` へ移す。
