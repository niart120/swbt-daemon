...
host が Dev Container 外であれば、recipe は `devcontainer up` と `devcontainer exec` で同名の in-container recipe を実行する。
Dev Container 内であれば、recipe は `cmake`、`ctest`、`scripts/format.sh`、`scripts/check-format.sh` を直接実行する。

Windows host では POSIX shell の存在を前提にしない。
`justfile` は `windows-shell`、`[windows]`、`[unix]`、または OS 別 helper script を使い、Windows host の委譲処理を PowerShell で表現できる形にする。
...
- Windows host から `devcontainer exec` するときの quoting と environment variable 受け渡しを、`justfile` だけで安定させるか、PowerShell helper script に分離するか。
...
- Git hooks を Windows host でも同じ tracked hook で動かすか、hook body から OS 別 helper script に分岐するか。

## 8. 移行手順

1. Dev Container image に `just` を追加する。
2. `justfile` を追加し、既存 `Makefile` の public target と同じ recipe を実装する。
3. host からの `just <recipe>` が Dev Container CLI へ委譲されることを Linux、macOS、Windows host の前提で設計する。
...
5. `.github/workflows/ci.yml` の `runCmd` を `just verify-ci` に切り替える。
6. `.githooks/pre-commit` と `.githooks/pre-push` を `just` 呼び出しに切り替える。
7. `README.md`、`AGENTS.md`、`.githooks/README.md`、`.agents/skills/` のコマンド例を `just` に更新する。
8. `scripts/require-dev-environment.sh` の案内から Makefile 前提の文言を外す。
9. `Makefile` を shim にするか削除する。
10. `spec/operations/development-tooling.md` を `just` 前提の current spec に更新し、この draft spec を current または superseded として整理する。

## 9. 現行記述の修正方針

| path | 現行の問題 | 修正方針 |
|---|---|---|
| `AGENTS.md` | 主開発環境を WSL2 + Dev Containers とし、標準入口を Makefile target としている。 | 主開発環境を Linux/macOS/Windows host + Dev Container と書き、標準入口を `just` recipe にする。 |
| `README.md` | WSL2 前提、`make` コマンド例、Makefile が host から Dev Container CLI へ委譲する説明がある。 | 開発者の host OS を限定しない説明に変え、command examples を `just debug` などにする。 |
...
| `.githooks/README.md` | hook の説明が Makefile 前提である。 | hook の標準入口を `just` に変更する。 |
...
- host から `just debug` が host toolchain を使わず Dev Container CLI へ委譲する。
- Windows host から `just debug` が POSIX shell なしで Dev Container CLI へ委譲する。
