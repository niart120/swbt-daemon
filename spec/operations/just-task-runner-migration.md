...
host 側の `just` は、Linux、macOS、WSL2 shell、`local_032` で採用すると判断した場合の Windows native PowerShell から開発者が叩く外側の入口である。
...
6. Windows native PowerShell からの `just <recipe>` は、`work-units/wip/local_032/WINDOWS_NATIVE_JUST_DEVCONTAINER.md` で実行可否を確認してから標準入口に含めるか判断する。
...
11. `Makefile` を shim にするか削除する。
12. `spec/operations/development-tooling.md` を `just` 前提の current spec に更新し、この draft spec を current または superseded として整理する。
13. 置き換え済みの current spec が残る場合は、`状態` を `superseded` にして `spec/archive/` へ移す。
...
| `spec/operations/development-tooling.md` | Makefile target を標準検証入口としている。 | `just` 移行完了後に current spec として更新する。別 spec に置き換える場合は `superseded` として `spec/archive/` へ移す。 |
...
- Windows native PowerShell から `just debug` が Dev Container CLI へ委譲できるかを確認し、未確認のまま標準入口としない。
