...
current implementation は `swbt-daemon.exe` の引数なし起動で production backend を選ぶ。Bluetooth adapter を開かない確認では `--backend noop` を明示する。`SWBT_RUN_HARDWARE=1` と `SWBT_HARDWARE_APPROVED=1` は current implementation の実装上の分岐条件ではない。
