...
## 2026-06-27: device event listener 方針の先送り

### 現状

`local_080` は production の report send path を `swbt_btstack_device_send` 経由へ寄せる work unit として扱う。

### 観測

device event listener registration は packet handling の抽象として候補になるが、listener lifetime、同期処理時間、将来の queueing 判断まで範囲が広がる。

### 判断

event listener は `local_080` の対象外にする。production send path 整理後も packet handler 境界が問題として残る場合だけ、別 work unit の source として再検討する。

## 2026-06-28: active reconnect automatic retry の保留

### 現状

`local_101` は、active reconnect の `Create_connection` 後に `Connection_complete` が来ない failure を `production: active reconnect timeout` として表面化した。

### 観測

同じ source artifact の手動 retry で HID open へ進む run はあるが、未完了の HCI Create Connection を daemon 内部で安全に cancel / retry できるかは未監査である。

### 判断

automatic retry は現時点で実装しない。必要になった場合だけ、BTstack / controller 側の cancel 可否、retry 対象、最大回数、shutdown 中の扱いを source audit してから work unit 化する。
