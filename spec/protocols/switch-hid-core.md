...
current dispatcher は `SET_REPORT_MODE`、`ENABLE_IMU`、`ENABLE_VIBRATION` に simple ACK を返し、`SPI_FLASH_READ` に virtual SPI read reply を返し、`SET_PLAYER_LIGHTS` / `GET_PLAYER_LIGHTS` を player lights state に接続する。
...
- request device info、manual pairing、regulated voltage の reply data は未実装である。
